  ...
  const profile: UserProfile | null =
    userReady && portfolioReady && userDoc && portfolioDoc
      ? {
          ...
          avatarFrame: (userDoc.avatarFrame as any) ?? 'none',
          bio: userDoc.bio,
          country: userDoc.country ?? 'India',
          city: userDoc.city ?? '',
          tradingExperience: userDoc.tradingExperience ?? 'Intermediate',
          favouriteMarket: userDoc.favouriteMarket ?? 'Indian Equities',
          favouriteSector: userDoc.favouriteSector ?? 'AI & Tech',
          avatarVisibility: userDoc.avatarVisibility ?? 'public',
          isPortfolioPublic: userDoc.isPortfolioPublic ?? true,
          badges: userDoc.badges ?? [],
          isPostingSuspended: userDoc.isPostingSuspended ?? false,
